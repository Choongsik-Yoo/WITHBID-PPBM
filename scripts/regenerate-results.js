...
import {
  quoteWorkbookBuffer,
  reportToDashboardHtml,
} from "../src/lib/result-artifacts.js";

const config = getConfig();
const state = await readJson(
  path.join(config.dataRoot, "_데이터베이스", "app-state.json"),
  { notices: [] },
);
...
for (const notice of state.notices) {
  ...
  const [report, extraction] = await Promise.all([
    readJson(path.join(base, "AI_판단결과.json"), null),
    readJson(structured, {}),
  ]);
  if (!report) continue;

  await fs.writeFile(
    path.join(base, "입찰참가판단_대시보드.html"),
