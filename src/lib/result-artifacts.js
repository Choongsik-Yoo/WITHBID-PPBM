...
export function reportToDashboardHtml(notice, report, extraction = {}) {
  ...
  const incomplete = configuration.filter((item) => item.unitPrice == null || !(Number(item.quantity) > 0));
  ...
  const source = (value) => /^https:\/\//i.test(value || "") ? `<a href="${esc(value)}" target="_blank" rel="noreferrer">${/\/search/i.test(value) ? "검색 링크" : "상품 링크"} ↗</a>` : esc(value);
  ...
  </style></head><body><main class="wrap"><section class="hero"><div class="eyebrow">WITHBID-PPBM · PROCUREMENT DECISION</div><h1>${esc(notice.title)}</h1><div class="decision ${decisionClass}">${esc(report.decision)}</div><p class="summary">${esc(report.summary)}</p></section><section class="grid"><div class="card"><div class="label">공고번호</div><div class="value">${esc(notice.noticeNumber)}</div></div><div class="card"><div class="label">수요기관</div><div class="value">${esc(notice.organization || "확인 필요")}</div></div><div class="card"><div class="label">마감일</div><div class="value">${esc(notice.deadline)}</div></div><div class="card"><div class="label">예산 / 견적 원가</div><div class="value">${won(extraction.budget)} / ${incomplete.length ? `미확인 (${incomplete.length}개 항목)` : won(total || null)}</div></div></section><section class="columns"><div><article class="panel"><h2>자격·인증 검토</h2><ul>${list(report.qualificationReview)}</ul></article><article class="panel"><h2>주요 리스크</h2><ul>${list(report.risks)}</ul></article></div><article class="panel"><h2>실무 확인 체크리스트</h2><ul>${list(report.checklist)}</ul></article></section><article class="panel"><h2>사양별 부품 구성 견적</h2><div class="table-wrap"><table><thead><tr><th>사양 그룹</th><th>구분</th><th>요구사양</th><th>선정 모델</th><th>단가</th><th>전체수량</th><th>가격 출처</th><th>상태</th></tr></thead><tbody>${rows}</tbody></table></div></article><footer>완제품 본체 가격은 제외하고, 사양별 구성 부품 원가를 합산한 결과입니다. 미확인 항목이 있으면 총액을 확정하지 않으며 최종 투찰 전 원문과 담당자 검토가 필요합니다.</footer></main></body></html>`;
}

const componentTemplate = [
  ["CASE", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["MAINBOARD", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["CPU", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["CPU 쿨러", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["RAM", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["M.2", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["HDD", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["POWER", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["VGA", { requirement:null, model:null, unitPrice:null, quantity:null }],
  ["운영체제(O/S)", { requirement:null, model:null, unitPrice:null, quantity:null }],
...
const requiredComputerCategories = new Set(["CASE", "MAINBOARD", "CPU", "CPU 쿨러", "RAM", "M.2", "POWER"]);
...
export async function quoteWorkbookBuffer(notice, report, extraction = {}, targetMargin = 12) {
  ...
  for (const detail of details) {
    ...
    const start=10;
    rows.forEach((item,index)=>{const r=start+index;const isLink=/^https:\/\//i.test(item.source||"");const linkLabel=/\/search/i.test(item.source||"")?"검색 링크":"상품 링크";sheet.getRow(r).values=[item.category,item.requirement,item.selectedModel,item.unitPrice,item.unitQuantity,{formula:`IF(OR(D${r}="",E${r}=""),"",D${r}*E${r})`},isLink?{text:linkLabel,hyperlink:item.source}:item.source,item.status];sheet.getRow(r).height=34;sheet.getRow(r).alignment={vertical:"middle",wrapText:true};if(isLink)sheet.getCell(r,7).font={color:{argb:"FF0563C1"},underline:true};});
    const end=start+rows.length-1; const subtotal=end+1; const groupTotal=subtotal+1; const vat=groupTotal+1; const grand=vat+1; const missing=grand+1;
    sheet.mergeCells(subtotal,1,subtotal,5); sheet.getCell(subtotal,1).value="1대당 확인 원가"; sheet.getCell(subtotal,6).value={formula:`IF(COUNT(F${start}:F${end})=0,"",SUM(F${start}:F${end}))`};
    sheet.mergeCells(groupTotal,1,groupTotal,5); sheet.getCell(groupTotal,1).value=`공급가액 (${group.systemQuantity ?? "수량 확인 필요"}대)`; sheet.getCell(groupTotal,6).value={formula:`IF(OR(F${subtotal}="",E4="",F${missing}>0),"",F${subtotal}*E4)`};
    sheet.mergeCells(vat,1,vat,5); sheet.getCell(vat,1).value="부가세"; sheet.getCell(vat,6).value={formula:`IF(F${groupTotal}="","",F${groupTotal}*10%)`};
    sheet.mergeCells(grand,1,grand,5); sheet.getCell(grand,1).value="사양별 확인 합계"; sheet.getCell(grand,6).value={formula:`IF(OR(F${groupTotal}="",F${vat}=""),"",F${groupTotal}+F${vat})`};
    const missingFormula=rows.map((item,index)=>(item.requirement || requiredComputerCategories.has(item.category))?`IF(OR(B${start+index}="",D${start+index}="",E${start+index}=""),1,0)`:null).filter(Boolean).join("+")||"0";
    sheet.getCell(missing,1).value="미확인 단가/수량"; sheet.getCell(missing,6).value={formula:missingFormula};
    ...
    detailRefs.push({ group, sheetName, subtotal, groupTotal, vat, grand, missing });
  }
  ...
  const totalsReady=`AND(SUM(H${summaryStart}:H${summaryEnd})=0,COUNT(G${summaryStart}:G${summaryEnd})=${detailRefs.length})`;
  summary.mergeCells(totalRow,1,totalRow,4); summary.getCell(totalRow,1).value="전체 공급가액"; summary.getCell(totalRow,5).value={formula:`IF(${totalsReady},SUM(E${summaryStart}:E${summaryEnd}),"")`};
  summary.mergeCells(totalRow+1,1,totalRow+1,4); summary.getCell(totalRow+1,1).value="전체 부가세"; summary.getCell(totalRow+1,6).value={formula:`IF(${totalsReady},SUM(F${summaryStart}:F${summaryEnd}),"")`};
  summary.mergeCells(totalRow+2,1,totalRow+2,4); summary.getCell(totalRow+2,1).value="전체 견적 합계"; summary.getCell(totalRow+2,7).value={formula:`IF(${totalsReady},SUM(G${summaryStart}:G${summaryEnd}),"")`};
  summary.mergeCells(totalRow+3,1,totalRow+3,4); summary.getCell(totalRow+3,1).value="예상매출(목표마진)"; summary.getCell(totalRow+3,7).value={formula:`IF(OR(G${totalRow+2}="",E6>=100%),"",G${totalRow+2}/(1-E6))`};
  summary.mergeCells(totalRow+4,1,totalRow+4,4); summary.getCell(totalRow+4,1).value="예산대비 여유"; summary.getCell(totalRow+4,7).value={formula:`IF(OR(B6="",G${totalRow+3}=""),"",B6-G${totalRow+3})`};
  ...
  report.qualificationReview.forEach((v)=>notes.addRow(["자격·인증",v]));report.quoteAudit?.forEach((v)=>notes.addRow([`견적검증/${v.severity === "blocking" ? "필수" : "확인"}`,v.message]));report.risks.forEach((v)=>notes.addRow(["리스크",v]));report.checklist.forEach((v)=>notes.addRow(["체크리스트",`□ ${v}`]));extraction.uncertainties?.forEach((v)=>notes.addRow(["불확실성",v]));report.configuration.filter((v)=>v.evidence || v.evidenceLocations?.length).forEach((v)=>notes.addRow(["원문근거",`${v.specificationGroup || "공통"} · ${v.category}: ${v.evidence || ""}${v.evidenceLocations?.length ? ` (${v.evidenceLocations.join(", ")})` : ""}`]));notes.eachRow((row,index)=>{row.alignment={vertical:"top",wrapText:true};if(index>1)row.height=35;});
