...
export async function quoteWorkbookBuffer(notice, report, extraction = {}, targetMargin = 12) {
  ...
  for (const detail of details) {
    ...
    const start=10;
    rows.forEach((item,index)=>{const r=start+index;sheet.getRow(r).values=[item.category,item.requirement,item.selectedModel,item.unitPrice,item.unitQuantity,{formula:`IF(OR(D${r}="",E${r}=""),"",D${r}*E${r})`},/^https:\/\//i.test(item.source||"")?{text:"상품 링크",hyperlink:item.source}:item.source,item.status];sheet.getRow(r).height=34;sheet.getRow(r).alignment={vertical:"middle",wrapText:true};if(/^https:\/\//i.test(item.source||""))sheet.getCell(r,7).font={color:{argb:"FF0563C1"},underline:true};});
