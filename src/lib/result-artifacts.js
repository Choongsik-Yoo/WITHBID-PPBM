...
export function reportToDashboardHtml(notice, report, extraction = {}) {
  ...
  const rows = report.configuration.map(item => `<tr><td>${esc(item.category)}</td><td>${esc(item.requirement)}</td><td>${esc(item.selectedModel || "확인 필요")}</td><td class="num">${item.unitPrice == null ? "-" : won(item.unitPrice)}</td><td class="num">${esc(item.quantity ?? "-")}</td><td>${esc(item.source)}</td><td><span class="tag">${esc(item.status)}</span></td></tr>`).join("");
...
export async function quoteWorkbookBuffer(notice, report, extraction = {}, targetMargin = 12) {
  ...
  const start=10;
  report.configuration.forEach((item,index)=>{const r=start+index; sheet.getRow(r).values=[item.category,item.requirement,item.selectedModel||"확인 필요",item.unitPrice??null,item.quantity??null,{formula:`IF(OR(D${r}="",E${r}=""),0,D${r}*E${r})`},{formula:`F${r}*10%`},{formula:`F${r}+G${r}`},item.source,item.status]; sheet.getRow(r).alignment={vertical:"top",wrapText:true}; sheet.getRow(r).height=42;});
