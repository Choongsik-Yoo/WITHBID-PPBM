...
3. 미등록 품목은 컴퓨존 → 가이드컴 순으로 확인하고, 두 사이트에 없으면 "다나와 확인 필요"로 표시한다.
4. 가격 조사는 사양 그룹별 CPU → MAINBOARD → CPU 쿨러 → RAM → M.2 → HDD → VGA → POWER → CASE → 운영체제 → 그 외 순서로 하며, 앞서 선정한 부품과의 호환성을 확인한다.
5. 외부 가격이 사용자에 의해 제공되면 판매처, 조회 시각, 상품 상세 링크를 함께 표시한다.
6. 부가세 포함 여부와 배송비가 불명확하면 별도 확인 항목으로 둔다.
