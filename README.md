...
`npm run package:windows`를 실행하면 `dist/WITHBID-PPBM-Desktop-Setup-0.3.2.zip`이 생성됩니다. 사용자 PC에서 압축을 푼 뒤 `Install-WITHBID-PPBM.cmd`를 더블클릭하면 설치 진행 창이 표시되고 사용자별 앱 폴더와 바탕화면/시작 메뉴 바로가기가 설치됩니다. Node.js 실행파일이 패키지에 포함되므로 사용자 PC에 Node.js를 별도로 설치할 필요가 없습니다.
